...
facet_to_pylist_by_index(const facetT* facet)
{
	...
	if (facet_as_list == NULL)
		return NULL;
	FOREACHvertex_(facet->vertices)	{
		node_index = PyLong_FromLong(qh_pointid(vertex->point));
		if (node_index == NULL)
			goto cleanup;
...
vertex_find_neighbours(const vertexT *myvertex)
{
	...
	FOREACHneighbor_(myvertex) {
		...
		FOREACHvertex_(neighbor->vertices) {
			...
			if (neighbour == NULL)
				goto cleanup;
			if (PyObject_RichCompareBool(myvertex_as_pytuple, neighbour, Py_NE)
...
PyMODINIT_FUNC PyInit__qhull(void)
{
