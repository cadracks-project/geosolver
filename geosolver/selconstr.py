...
from geosolver.constraint import Constraint
...
class NotCounterClockwiseConstraint(SelectionConstraint):
    """select triplets that are not counter clockwise
...
class NotClockwiseConstraint(SelectionConstraint):
    """select triplets that are not clockwise
    (counterclockwise or degenerate)"""
...
class FunctionConstraint(SelectionConstraint):
    """select solutions where function returns true when applied
    to given variables"""
    ...
    def satisfied(self, map):
        """return True iff given solution (map) for given variables applied
        to function gives True"""
        values = []
