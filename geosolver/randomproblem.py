...
from geosolver.geometric import GeometricProblem, DistanceConstraint, AngleConstraint
...
def _constraint_group(problem, group, dependend, angleratio):
    ...
    diag_print("_constraint_group(group=" + str(group.keys()) + ",dep=" + str(
        dependend) + ")", "geometric._constraint_group")
    if len(group) == 2:
        if dependend == None:
            v1 = group.keys()[0]
            v2 = group.keys()[1]
