...
def add_problem():
    ...
    problem.add_point('v2', vector([random.random() for i in [1, 2]]))
    problem.add_point('v3', vector([random.random() for i in [1, 2]]))
    problem.add_constraint(DistanceConstraint(
        'v1',
        'v2',
        distance_2p(problem.get_point('v1'),
                    problem.get_point('v2'))
    ))
    problem.add_constraint(DistanceConstraint(
        'v2',
        'v3',
        distance_2p(problem.get_point('v2'),
                    problem.get_point('v3'))
    ))
    problem.add_constraint(AngleConstraint(
        'v3',
        'v1',
        'v2',
        angle_3p(problem.get_point('v3'),
...
def ada_problem():
    ...
    problem.add_point('v2', vector([random.random() for i in [1,2]]))
    problem.add_point('v3', vector([random.random() for i in [1,2]]))
    problem.add_constraint(DistanceConstraint('v1',
                                              'v2',
                                              distance_2p(problem.get_point('v1'),
                                                          problem.get_point('v2'))))
    problem.add_constraint(AngleConstraint('v3', 'v1', 'v2',
...
def test_fix(n):
    ...
    print("number of top clusters: %i" % len(drplanner.dr.top_level()))
...
def test_fix2(n):
    ...
    print("number of top clusters: %i" % len(drplanner.dr.top_level()))
    print("top clusters: %s" % map(str, drplanner.dr.top_level()))
...
def test_ada_3d():
    ...
    print("result:")
    print(result)
    print("result is %s with %i solutions" % (result.flag, len(result.solutions)))
    check = True
...
if __name__ == "__main__":
    ...
    # test(random_distance_problem_3D(15, 1.0, 0.0))
    # stats_solving()
    # stats_incremental()
