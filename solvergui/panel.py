...
class Panel(QtWidgets.QWidget):
    ...
    def renameItem(self, oldName, newName):
        itemList = self.ui.treeWidget.findItems(oldName,
    ...
    def removeItem(self, name):
        itemList = self.ui.treeWidget.findItems(name,
                                                QtCore.Qt.MatchCaseSensitive | QtCore.Qt.MatchRecursive,
                                                0)
        if len(itemList) > 0:
    ...
    def keyPressEvent(self, keyEvent):
        if keyEvent.key() == QtCore.Qt.Key_Delete:
            self.prototypeManager.deleteObject(
                self.prototypeManager.objectSelected)
            self.mainWindow.updateWindow()
