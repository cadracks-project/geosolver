...
class Viewport(QtWidgets.QScrollArea):
    def __init__(self, viewportMngr, vpType=None, shareWidget=None,
                 parent=None):
    ...
    def setViewport(self, vpType, shareWidget=None):
        self.setName(vpType)
        self.glViewport = GLViewport(self, vpType, shareWidget,
                                     QtOpenGL.QGLFormat(
    ...
    def setMoveActive(self, active):
        ...
        self.glViewport.cameraHandler.setActiveMouseaction(
    ...
    def createActions(self):
        ...
        self.actionZoom = QtWidgets.QAction(self.tr("Zoom"), self)
        self.actionZoom.setCheckable(True)
        self.actionRotate = QtWidgets.QAction(self.tr("Rotate"), self)
        self.actionRotate.setCheckable(True)
        self.actionSync = QtWidgets.QAction(self.tr("Sync"), self)
    ...
    def createToolbar(self):
        ...
        self.viewportList = QtWidgets.QComboBox()
    ...
    def createVerticalSplit(self):
        ...
        self.splitterVertical.setFrameStyle(
    ...
    def createTriggers(self):
        ...
        #                        self.setMoveActive)
        # QtCore.QObject.connect(self.actionRotate,
        #                        QtCore.SIGNAL("triggered(bool)"),
        #                        self.setRotateActive)
        # QtCore.QObject.connect(self.actionZoom,
        #                        QtCore.SIGNAL("triggered(bool)"),
        #                        self.setZoomActive)
        # QtCore.QObject.connect(self.actionSync,
        #                        QtCore.SIGNAL("triggered(bool)"), self.doSync)
    ...
    def changeViewport(self, viewportName):
        if self.name == viewportName:
            ...
            if viewportName == "Top":
                self.glViewport = GLViewport(self, ViewportType.TOP, None,
                                             QtOpenGL.QGLFormat(
                                                 QtOpenGL.QGL.SampleBuffers))
            elif viewportName == "Side":
                self.glViewport = GLViewport(self, ViewportType.SIDE, None,
                                             QtOpenGL.QGLFormat(
                                                 QtOpenGL.QGL.SampleBuffers))
            elif viewportName == "Front":
                self.glViewport = GLViewport(self, ViewportType.FRONT, None,
                                             QtOpenGL.QGLFormat(
                                                 QtOpenGL.QGL.SampleBuffers))
            elif viewportName == "Perspective":
                self.glViewport = GLViewport(self, ViewportType.PERSPECTIVE,
                                             None, QtOpenGL.QGLFormat(
                        QtOpenGL.QGL.SampleBuffers))
            elif viewportName == "Decomposition":
                self.glViewport = CompositionView(self, self.viewportManager,
                                                  ViewportType.DECOMPOSITION,
                                                  PrototypeManager())  # self.getMainWindow().compositionView #
