...
class SceneObject:
    ...
    def drawCylinder(self, orientation, position, radiusBottom, radiusTop,
                     height):
        glPushMatrix()
        glColor3fv([0.0, 1.0, 1.0])
        glTranslatef(position[0], position[1], position[2])
        glRotatef(orientation[0], orientation[1], orientation[2],
                  orientation[3])
    ...
    def drawRectangle(self, geometry):
        ...
        glRecti(geometry.left(), geometry.top(), geometry.right(),
                geometry.bottom())
    ...
    def drawBackgroundgrid3D(self, gridWidth, gridHeight, rectWidth, rectHeight,
                             gridColor, filled=False):
        if rectWidth > 0 and rectHeight > 0:
            ...
            for x in range(0, rangeX):
                ...
                for z in range(0, rangeZ):
                    glColor3f(gridColor.redF(), gridColor.greenF(),
                              gridColor.blueF())
                    if (filled):
                        if (x + z) % 2 == 0:
                            glColor3f(1, 1, 1)
                            ...
                            glColor3f(0, 0, 1)
                        glRectf(posX1, posZ1, posX1 + rectWidth,
                                posZ1 + rectHeight)
                    else:
            ...
            self.drawThickAxis(posX1Start, posX1Start + gridWidth + rectWidth,
                               0.0, 0.0, posZ1Start,
                               posZ1Start + gridHeight + rectHeight, 2.0)
            # glDisable(GL_LINE_SMOOTH)
            glEnable(GL_LIGHTING)
            glPopMatrix()

    def drawBackgroundgrid2D(self, rectWidth, rectHeight, gridColor,
                             filled=False):
        if rectWidth > 0 and rectHeight > 0:
            glPushMatrix()
            ...
            if vpType == ViewportType.FRONT:
                if filled:
                    self.drawFilledGrid(posLeft, posBottom, widthRange,
                                        heightRange, rectWidth, rectHeight)
                else:
                    self.drawOpenGrid(posLeft, posBottom, widthRange,
                                      heightRange, rectWidth, rectHeight,
                                      gridColor)
                self.drawThickAxis(posLeft, posLeft + viewWidth + rectWidth,
                                   posBottom,
                                   posBottom + viewHeight + rectHeight,
                                   0.0, 0.0, 2.0)
            elif vpType == ViewportType.SIDE:
                glPushMatrix()
                ...
                if filled:
                    ...
                    self.drawOpenGrid(posLeft, posBottom, widthRange,
                                      ...
                                      gridColor)
                glPopMatrix()
                self.drawThickAxis(0.0, 0.0, posBottom,
                                   posBottom + viewHeight + rectHeight,
                                   -posLeft, -(posLeft + viewWidth + rectWidth),
                                   2.0)
            elif vpType == ViewportType.TOP:
                glPushMatrix()
                glRotatef(90.0, 1.0, 0.0, 0.0)
                ...
                self.drawThickAxis(posLeft, posLeft + viewWidth + rectWidth,
                                   0.0, 0.0,
                                   posBottom,
                                   (posBottom + viewHeight + rectHeight), 2.0)

            glEnable(GL_LIGHTING)
            glPopMatrix()

    def drawFilledGrid(self, startPosX, startPosY, rangeX, rangeY, rectWidth,
                       rectHeight):
        posX = startPosX
        for x in range(0, rangeX):
            posY = startPosY
    ...
    def drawOpenGrid(self, startPosX, startPosY, rangeX, rangeY, rectWidth,
                     rectHeight, gridColor):
        glLineWidth(0.5)
        posX = startPosX
    ...
    def drawThickAxis(self, beginX, endX, beginY, endY, beginZ, endZ,
                      thickness):
