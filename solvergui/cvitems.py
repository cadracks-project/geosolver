...
class CVCluster(QtWidgets.QGraphicsItem, Node):
    ...
    def updateBound(self):
        ...
        pointBegin.setX((self.paintRect.width() / 2.0) * math.cos(
            math.radians(self.startAngle / 16.0)))
        pointBegin.setY((self.paintRect.height() / 2.0) * math.sin(
            math.radians(self.startAngle / 16.0)))

        pointEnd = QtCore.QPointF(0.0, 0.0)
        pointEnd.setX((self.paintRect.width() / 2.0) * math.cos(
            math.radians((self.startAngle + self.spanAngle) / 16.0)))
        pointEnd.setY((self.paintRect.height() / 2.0) * math.sin(
            math.radians((self.startAngle + self.spanAngle) / 16.0)))

        pointMiddle = QtCore.QPointF(0.0, 0.0)
        halfDegree = (self.startAngle + (self.spanAngle / 2.0)) / 16.0
        pointMiddle.setX(
            (self.paintRect.width() / 2.0) * math.cos(math.radians(halfDegree)))
        pointMiddle.setY((self.paintRect.height() / 2.0) * math.sin(
            math.radians(halfDegree)))
    ...
    def __translateBound(self):
        ...
        self.boundary.setLeft(
            self.boundary.left() + self.paintRect.center().x())
        self.boundary.setRight(
            self.boundary.right() + self.paintRect.center().x())
        self.boundary.setTop(-self.boundary.top() + self.paintRect.center().y())
        self.boundary.setBottom(
            -self.boundary.bottom() + self.paintRect.center().y())
    ...
    def paint(self, painter, option, widget):
        ...
        if self.flag != None:
            if self.flag == GeometricCluster.OK:
                painter.setBrush(
                    QtGui.QBrush(self.compositionView.wellConstrainedColor))
            elif self.flag == GeometricCluster.I_UNDER or self.flag == GeometricCluster.S_UNDER:
                painter.setBrush(
                    QtGui.QBrush(self.compositionView.underConstrainedColor))
            elif self.flag == GeometricCluster.I_OVER or self.flag == GeometricCluster.S_OVER:
                painter.setBrush(
                    QtGui.QBrush(self.compositionView.overConstrainedColor))
            elif self.flag == GeometricCluster.UNSOLVED:
                painter.setBrush(
                    QtGui.QBrush(self.compositionView.unsolvedColor))
    ...
    def setInfoOverlay(self):
        ...
        for point in self.variables:
            pointObject = self.compositionView.prototypeManager.getObjectByKey(
                point)
            if pointObject != None:
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == QtCore.Qt.LeftButton:
            ...
            if not self.clusterActive:
                ...
                self.compositionView.prototypeManager.removeClusterObjects(
                    [self.permCluster], True, True)
                self.clusterActive = False
                self.fixGraphic.hide()
    ...
    def hoverEnterEvent(self, event):
        ...
        if not self.clusterActive:
            self.compositionView.prototypeManager.selectObjectsByKeys(
                self.variables)
    ...
    def hoverMoveEvent(self, event):
        self.overlayPosition = self.compositionView.ui.graphicsView.mapFromScene(
            event.scenePos())
        self.compositionView.infoOverlay.setPosition(event.scenePos())
    ...
    def hoverLeaveEvent(self, event):
        ...
        if not self.clusterActive:
            ...
            self.compositionView.prototypeManager.removeClusterObjects(
                [self.clusterHighlight], True, False)
            self.compositionView.updateViewports()
    ...
    def highlightCluster(self):
        if self.clusterHighlight != None:
            if self.clusterHighlight.temporary:
                self.compositionView.prototypeManager.addClusterObject(
                    self.clusterHighlight)
            self.clusterHighlight.selected = True
        else:
            self.clusterHighlight = self.compositionView.prototypeManager.selectClusterObjectByKeys(
                self.variables)
            if self.clusterHighlight == None:
                self.clusterHighlight = self.compositionView.prototypeManager.createCluster(
                    self.variables)
                if self.clusterHighlight != None:
    ...
    def permanentCluster(self):
        if self.permCluster == None:
            self.permCluster = self.compositionView.prototypeManager.createCluster(
                self.variables, self.flag)
...
class CVPoint(QtWidgets.QGraphicsItem, Node):
    ...
    def __init__(self, compView, parentNode, id, parent=None):
        ...
        self.fixGraphic = QtWidgets.QGraphicsSimpleTextItem("F", self)
        self.fixGraphic.hide()
        self.bezierCurve = None
        self.flag = None
        self.prtRef = None
    ...
    def setInfoOverlay(self):
        ...
        if self.prtRef != None:
            ...
            constrInfo += "Position: (" + str(
                round(self.prtRef.position[0], 2)) + " , " + str(
                round(self.prtRef.position[1])) + " , " + str(
                round(self.prtRef.position[2])) + ")"
        self.compositionView.infoOverlay.infoText = constrInfo
...
class CVInfoOverlay(QtWidgets.QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        textRect = QtCore.QRect(self.infoOverlay.left() + 4,
                                self.infoOverlay.top(),
                                self.infoOverlay.width() - 3,
                                self.infoOverlay.height() - 3)

        painter.drawText(textRect, QtCore.Qt.AlignLeft | QtCore.Qt.TextWordWrap,
                         self.infoText)
        painter.restore()
...
class CVConnection(QtWidgets.QGraphicsItem):
    """ Visualisation of the connections between the clusters, where two
    types of visualisation can be chosen: Bezier(default) and Lines """
        ...
        if self.nodeFrom != None and self.nodeTo != None:
            ...
            if self.connectType == ConnectionType.BEZIER:
                ...
                if self.compositionView.tree.orientation == TreeOrientation.TOP:
                    ...
                    firstPoint = QtCore.QPointF(diffPosExt.x(),
                                                diffPosExt.height() * 0.5)
                elif self.compositionView.tree.orientation == TreeOrientation.BOTTOM:
                    self.beziercurve.moveTo(diffPosExt.x(), diffPosExt.bottom())
                    firstPoint = QtCore.QPointF(diffPosExt.x(),
                                                -diffPosExt.height() * 0.5 + self.nodeTo.paintRect.height())
                elif self.compositionView.tree.orientation == TreeOrientation.LEFT:
                    self.beziercurve.moveTo(diffPosExt.x(), diffPosExt.y())
                    firstPoint = QtCore.QPointF(diffPosExt.width() * 0.5,
                                                diffPosExt.y())
                elif self.compositionView.tree.orientation == TreeOrientation.RIGHT:
                    self.beziercurve.moveTo(diffPosExt.x(), diffPosExt.y())
                    firstPoint = QtCore.QPointF(
                        self.nodeTo.paintRect.width() + diffPosExt.width() * 0.5,
                        diffPosExt.y())

                if self.compositionView.tree.orientation == TreeOrientation.TOP:
                    secondPoint = QtCore.QPointF(diffPosExt.right(),
                                                 diffPosExt.height() * 0.5)
                elif self.compositionView.tree.orientation == TreeOrientation.BOTTOM:
                    secondPoint = QtCore.QPointF(diffPosExt.right(),
                                                 self.nodeTo.paintRect.height() - diffPosExt.height() * 0.5)
                elif self.compositionView.tree.orientation == TreeOrientation.LEFT:
                    secondPoint = QtCore.QPointF(diffPosExt.width() * 0.5,
                                                 diffPosExt.bottom())
                elif self.compositionView.tree.orientation == TreeOrientation.RIGHT:
                ...
                if self.compositionView.tree.orientation == TreeOrientation.TOP:
                    painter.drawLine(
                        QtCore.QPointF(self.nodeTo.paintRect.width() * 0.5,
                                       0.0),
                        QtCore.QPointF(endPoint.x(), endPoint.y()))
                elif self.compositionView.tree.orientation == TreeOrientation.BOTTOM:
                    painter.drawLine(
                        QtCore.QPointF(self.nodeTo.paintRect.width() * 0.5,
                                       self.nodeTo.paintRect.height()),
                        QtCore.QPointF(endPoint.x(), endPoint.y()))
                elif self.compositionView.tree.orientation == TreeOrientation.LEFT:
                    painter.drawLine(QtCore.QPointF(0.0,
                                                    self.nodeTo.paintRect.height() * 0.5),
                                     QtCore.QPointF(endPoint.x(), endPoint.y()))
                elif self.compositionView.tree.orientation == TreeOrientation.RIGHT:
                    painter.drawLine(
                        QtCore.QPointF(self.nodeTo.paintRect.width(),
                                       self.nodeTo.paintRect.height() * 0.5),
                        QtCore.QPointF(endPoint.x(), endPoint.y()))
    ...
    def areaBetweenNodes(self):
        ...
        if self.compositionView.tree.orientation == TreeOrientation.TOP:
            ...
            area.setBottom(
                -diffPos.y() + self.nodeFrom.paintRect.height() * 0.5)
        elif self.compositionView.tree.orientation == TreeOrientation.RIGHT:
