...
class GLViewport(QGLWidget):
    def __init__(self, viewport, vpType, shareWidget=None, format=None,
                 parent=None):
    ...
    def startOverlay(self, upward=False):
        ...
        if (upward):
            glOrtho(0, self.camera.getWindowWidth(), 0,
                    self.camera.getWindowHeight(), 0.1, 100.0)
        else:
            glOrtho(0, self.camera.getWindowWidth(),
                    self.camera.getWindowHeight(), 0.0, 0.1, 100.0)
    ...
    def drawGrid(self):
        if self.viewportType == ViewportType.PERSPECTIVE or self.viewportType == ViewportType.SOLUTION:
            self.sceneObjects.drawBackgroundgrid3D(600, 600,
                                                   self.settings.sketcherData.gridWidth,
                                                   self.settings.sketcherData.gridHeight,
                                                   self.gridColor, False)
        else:
            self.sceneObjects.drawBackgroundgrid2D(
                self.settings.sketcherData.gridWidth,
                self.settings.sketcherData.gridHeight, self.gridColor, False)

    def drawSelection(self):
        if self.currentTool != None and self.currentTool.multipleSelect and self.currentTool.selectionStarted:
            self.sceneObjects.drawSelection(self.currentTool.beginSelPoint.x(),
                                            self.currentTool.beginSelPoint.y(),
                                            self.currentTool.endSelPoint.x(),
                                            self.currentTool.endSelPoint.y())
            """glPushMatrix()
            glDisable(GL_LIGHTING)
    ...
    def mouseMoveEvent(self, mouseEvent):
        ...
        if self.currentTool == None:
            ...
            if self.currentTool.needPicking:
                ...
                self.currentTool.handleMouseMove(mouseEvent, self.camera,
                                                 self.viewportType, selection)
            else:
                self.currentTool.handleMouseMove(mouseEvent, self.camera,
                                                 self.viewportType)
    ...
    def mousePressEvent(self, mouseEvent):
        ...
        if self.currentTool != None:
            ...
            if self.currentTool.needPicking:
                selection = self.picking(mouseEvent.pos(), mouseEvent.buttons())
                self.currentTool.handleMousePress(mouseEvent, self.camera,
                                                  self.viewportType, selection)
            else:
                self.currentTool.handleMousePress(mouseEvent, self.camera,
                                                  self.viewportType)
    ...
    def startPicking(self, point):
        ...
        if self.currentTool != None and self.currentTool.multipleSelect and self.currentTool.selectionStarted:
            ...
            self.selectionRect = QtCore.QRect(self.currentTool.beginSelPoint,
                                              self.currentTool.endSelPoint)
            ...
            gluPickMatrix(centerX, centerY, width, height,
                          self.camera.getViewport())
        else:
            gluPickMatrix(point.x(), point.y(), self.selectRegionWidth,
                          self.selectRegionHeight, self.camera.getViewport())
...
class SolutionGLViewport(GLViewport):
    ...
    def drawGrid(self):
        if self.viewportType == ViewportType.PERSPECTIVE or self.viewportType == ViewportType.SOLUTION:
            self.sceneObjects.drawBackgroundgrid3D(600, 600,
                                                   self.settings.svData.gridWidth,
                                                   self.settings.svData.gridHeight,
                                                   self.gridColor, False)
        else:
            self.sceneObjects.drawBackgroundgrid2D(
                self.settings.svData.gridWidth, self.settings.svData.gridHeight,
                self.gridColor, False)
