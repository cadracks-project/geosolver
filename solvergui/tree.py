...
class Tree:
    ...
    def firstWalk(self, tree, node, level):
        ...
        if (node.getChildrenCount() == 0) or (level == tree.maxDepth):
            ...
            if leftSibling != None:
                node.prelim = leftSibling.prelim + tree.getNodeSize(
                    leftSibling) + tree.siblingSeperation
            else:
                node.prelim = 0.0
        else:
    ...
    def apportion(self, tree, node, level):
        ...
        while firstChild != None and firstChildLeftNeighbour != None and j <= k:
            ...
            totalGap = (
                       firstChildLeftNeighbour.prelim + modifierSumLeft + tree.getNodeSize(
                           firstChildLeftNeighbour) + tree.subtreeSeperation) - (
    ...
    def secondWalk(self, tree, node, level, posX, posY):
        if level <= tree.maxDepth:
            ...
            if node.getChildrenCount() != 0:
                self.secondWalk(tree, node.children[0], level + 1,
                                posX + node.modifier,
                                posY + maxSizeTmp + tree.levelSeperation)
...
class Node:
    ...
    def getChildrenCenter(self, tree):
        if len(self.children) > 0:
            return self.children[0].prelim + ((self.children[-1].prelim -
                                               self.children[
                                                   0].prelim) + tree.getNodeSize(
                self.children[-1])) / 2.0
        else:
            return 0.0
