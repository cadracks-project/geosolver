...
class CameraHandler(QtCore.QObject):
    ...
    def mouseMoveEvent(self, mouseEvent, camera):
        ...
        if self.activeMouseAction == None:
            ...
            if mouseEvent.buttons() == self.glViewport.getMouseState(
                    self.activeMouseAction):
            ...
            if mouseEvent.buttons() == self.glViewport.getMouseState(
                    self.activeMouseAction):
                self.handleMouseRotate(camera, mouseEvent.x(), mouseEvent.y(),
                                       dx, dy, proj)
    ...
    def spin(self, camera):
        self.rotateAroundPoint(camera, self.spinningQuaternion,
                               camera.getRevolveAroundPoint())
    ...
    def rotateAroundPoint(self, camera, spinQuat, point):
        ...
        translation = point + tempQuat.rotate(
            camera.getPosition() - point) - camera.getPosition()
        pos = camera.getPosition()
        pos[0] += translation[0]
        pos[1] += translation[1]
        pos[2] += translation[2]
