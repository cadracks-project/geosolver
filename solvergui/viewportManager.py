...
class ViewportManager:
    ...
    def showAll(self):
        ...
        splitterHorUp = QtWidgets.QSplitter()
        ...
        splitterHorDown = QtWidgets.QSplitter()
    ...
    def updateDecompositionFixed(self, id, fixed):
        if self.displayViewport == DisplayViewport.ALL:
            for viewport in self.viewportList:
                if viewport.glViewport != None and viewport.glViewport.viewportType == ViewportType.DECOMPOSITION:
                    viewport.glViewport.updateState(id, fixed,
                                                    viewport.glViewport.tree.root)
        if self.mainWindow.compositionView != None:
            self.mainWindow.compositionView.updateState(id, fixed,
                                                        self.mainWindow.compositionView.tree.root)
    ...
    def minmaxView(self):
        if self.displayViewport == DisplayViewport.ALL:
            self.showViewport(DisplayViewport.SINGLE,
                              self.activeViewport.glViewport.viewportType)
        elif self.displayViewport == DisplayViewport.SINGLE:
