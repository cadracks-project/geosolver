...
class Command:
    """ Commands that are general to the whole scene can be subclassed
    from this class. """
    def __init__(self):
        """ initialize """
        pass
...
class UpdateTaskbarCommand(Command):
    """ Update the taskbar whena mouse move event occurs in on of the many
    views/viewports. """
    ...
    def execute(self, mouseEvent):
        ...
        translation = Vec(camera.unprojectedCoordinatesOf(
        ...
        text = "x:%s, y:%s, z:%s" % (position[0], position[1], position[2])
